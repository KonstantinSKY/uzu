...
use backend_uzu::{
    Array, DataType, ParameterLoader,
    backends::{
...
fn test_parameter_loader_basic() {
    ...
    let embeddings = unsafe {
        Array::from_allocation(embeddings_leaf.read_allocation().unwrap(), 0, &embedding_shape, DataType::BF16)
    };
    ...
    let same =
        unsafe { Array::from_allocation(weights_leaf.read_allocation().unwrap(), 0, &embedding_shape, DataType::BF16) };
