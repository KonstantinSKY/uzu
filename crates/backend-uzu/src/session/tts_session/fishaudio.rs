...
impl<B: Backend> FishAudioSemanticBridge<B> {
    fn load(
        ...
        let codebook_embeddings = parameter_tree
            .leaf("text_decoder.codebook_embeddings.weights")
            .and_then(|leaf| leaf.validate(&codebook_embeddings_shape, data_type))
        ...
        let fast_model_projection = if config.fast_model_projection_config.is_some() {
            Some(
                parameter_tree
                    .leaf("text_decoder.fast_model_projection.weights")
...
impl<B: Backend> FishAudioTextDecoderRuntime<B> {
    ...
    fn encode_slow_codebook_sum_from_gpu_tokens_on(
        ...
        first_code: u32,
        source_runner: &mut TokenDecoderRunner<B>,
        source_slot: usize,
        num_codebooks: usize,
        codebook_size: usize,
        slow_model_dim: usize,
        ...
        if residual_count > 0 {
            ...
            source_runner.copy_async_chain_results_to_on(
                encoder,
                source_slot,
                &mut semantic_bridge.codebook_token_indices,
                dst_byte_offset,
                residual_count,
            )?;
        }
